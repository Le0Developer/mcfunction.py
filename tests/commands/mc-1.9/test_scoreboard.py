...
from mcfunction.versions.mc_1_9.scoreboard import (
...
def test_tag_add():
    parsed = scoreboard.parse('scoreboard players tag @s add testtag')
    parsed: ParsedScoreboardCommand

    assert isinstance(parsed.target, EntityNode)
    assert parsed.mode.value == 'add'
    assert parsed.name.value == 'testtag'

    assert str(parsed) == 'scoreboard players tag @s add testtag'


def test_tag_list():
    parsed = scoreboard.parse('scoreboard players tag @s list')
    parsed: ParsedScoreboardCommand

    assert parsed.mode.value == 'list'

    assert str(parsed) == 'scoreboard players tag @s list'
