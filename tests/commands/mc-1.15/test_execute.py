...
class TestConditions:
    ...
    def test_store_storage(self):
        condition, = execute.parse(
            'execute store success storage test:storage a.b.c byte 1 run help'
        ).conditions
        condition: ParsedExecuteCondition

        assert condition.mode.value == 'storage'
        assert condition.destination.namespace == 'test'
        assert condition.destination.name == 'storage'

        assert str(condition) == 'store success storage test:storage a.b.c ' \
                                 'byte 1'
    ...
    def test_if_predicate(self):
        condition, = execute.parse(
            'execute if predicate test:predicate run help'
        ).conditions
        condition: ParsedExecuteCondition

        assert condition.condition.value == 'predicate'
        assert condition.predicate.namespace == 'test'
        assert condition.predicate.name == 'predicate'

        assert str(condition) == 'if predicate test:predicate'
