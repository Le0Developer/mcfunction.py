...
class ParsedTitleCommand(ParsedCommand):
    ...
    def __str__(self):
        ...
        if self.action.value in ('clear', 'reset'):
            return base
        elif self.action.value in ('title', 'subtitle'):
            ...
            raise ConstructionException(
                ...
                f'\'subtitle\', or \'times\', not {self.action.value!r}'
            )
...
# title <targets> (title|subtitle) <title>
title.add_variation(
    Parser(Entity(), 'target'),
    Parser(Union('title', 'subtitle'), 'action'),
    Parser(JSON(), 'title'),
)
